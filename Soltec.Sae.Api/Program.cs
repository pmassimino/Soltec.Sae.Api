...
app.MapGet("/api/almacen/articulo", (bool? soloActivos) =>
{
    ...
    {
        FiltrarActivos = soloActivos ?? false // Si no se env�a, por defecto es false
    };
...
app.MapGet("/api/ventas/Factura/view", (HttpRequest request, HttpResponse response, IMemoryCache cache) =>
{
    ...
    // Define una clave �nica para el cach�, basada en los par�metros de la solicitud
    string cacheKey = $"FacturaView_{fecha.ToShortDateString()}_{fechaHasta.ToShortDateString()}";

    // Intenta obtener los datos desde el cach�
    if (!cache.TryGetValue(cacheKey, out List<FacturaView> result))
    {
        // Si no se encuentra en cach�, realiza la consulta y almacena el resultado en cach� durante un tiempo espec�fico
        var cacheEntryOptions = new MemoryCacheEntryOptions
        {
            AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(300) // Ajusta el tiempo de expiraci�n seg�n tus necesidades
        };
        ...
        // Almacena los resultados en cach�
...
app.MapGet("/api/ventas/Factura/view/xls1", (HttpRequest request, HttpResponse response, IMemoryCache cache) =>
{
    ...
    using (MemoryStream stream = new MemoryStream())
    {
        ...
        {
            // Crear una hoja de c�lculo en el libro de trabajo
...
app.MapGet("/api/contabilidad/librobanco", (HttpRequest request, HttpResponse response) =>
{
    ...
    var fechaHastaStr = request.Query["FechaHasta"].ToString();
    var fechaHasta = fechaHastaStr == "" ? DateTime.Now : DateTime.ParseExact(fechaHastaStr, "MM-dd-yyyy", null);
    // 1. Intentamos obtener el valor de "FiltraConciliado". Si no existe o no es un bool v�lido, por defecto ser� 'false'.
    ...
    // 2. Intentamos obtener el valor de "Conciliado". Si no existe o no es un bool v�lido, por defecto ser� 'false'.
...
app.MapPost("/api/cereales/boleto", async (Boleto boletoRequest) =>
{
    try
    {
        ...
        if (errores.Any())
            return Results.BadRequest(new { error = "Errores de validaci�n", detalles = errores });
